...
table = (
    ...
    ( 'CMakeLists.txt'
        , r'set\W+optional_fun_lite_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( optional_fun_lite_version "{major}.{minor}.{patch}" )\n' )

#    , ( 'example/cmake-pkg/CMakeLists.txt'
#        , r'set\W+optional_fun_lite_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( optional_fun_lite_version "{major}.{minor}" )\n' )
#
#    , ( 'script/install-xxx-pkg.py'
#        , r'\optional_fun_lite_version\s+=\s+"([0-9]+\.[0-9]+\.[0-9]+)"\s*$'
#        , 'optional_fun_lite_version = "{major}.{minor}.{patch}"\n' )

    , ( 'include/nonstd/optional-fun.hpp'
        , r'\#define\s+optional_fun_lite_MAJOR\s+[0-9]+\s*$'
        , '#define optional_fun_lite_MAJOR  {major}' )

    , ( 'include/nonstd/optional-fun.hpp'
        , r'\#define\s+optional_fun_lite_MINOR\s+[0-9]+\s*$'
        , '#define optional_fun_lite_MINOR  {minor}' )

    , ( 'include/nonstd/optional-fun.hpp'
        , r'\#define\s+optional_fun_lite_PATCH\s+[0-9]+\s*$'
        , '#define optional_fun_lite_PATCH  {patch}\n' )
)
